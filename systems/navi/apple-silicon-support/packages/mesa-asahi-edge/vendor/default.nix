{
  ...
  enableTeflon ?
    stdenv.isLinux
    && stdenv.isAarch64, # currently only supports aarch64 SoCs, may change in the future
...
let
  ...
  copyRustDep = dep: ''
    cp -R --no-preserve=mode,ownership ${
      fetchCrate dep
    } subprojects/${dep.pname}-${dep.version}
  ...
  self = stdenv.mkDerivation {
    ...
    outputs =
      ["out" "dev" "drivers"]
      ++ lib.optionals enableOSMesa ["osmesa"]
      ++ lib.optionals stdenv.isLinux ["driversdev"]
      ++ lib.optionals enableTeflon ["teflon"]
      ++ lib.optionals enableOpenCL ["opencl"]
    ...
    mesonFlags =
      [
        ...
        (lib.mesonOption "gallium-drivers"
          (lib.concatStringsSep "," galliumDrivers))
        (lib.mesonOption "vulkan-drivers"
          (lib.concatStringsSep "," vulkanDrivers))
        ...
        (lib.mesonEnable "microsoft-clc"
          false) # Only relevant on Windows (OpenCL 1.2 API on top of D3D12)

        # To enable non-mesa gbm backends to be found (e.g. Nvidia)
        (lib.mesonOption "gbm-backends-path"
          "${libglvnd.driverLink}/lib/gbm:${placeholder "out"}/lib/gbm")
      ...
      ++ lib.optionals (!withValgrind) [(lib.mesonEnable "valgrind" false)]
      ++ lib.optionals (!withLibunwind) [(lib.mesonEnable "libunwind" false)]
    ...
    buildInputs = with xorg;
      ...
      [elfutils]
      ++ lib.optionals enableOpenCL [llvmPackages.clang]
    ...
    propagatedBuildInputs =
      (with xorg; [libXdamage libXxf86vm])
